        ...
        {PARTNERS.map((partner, index) => <motion.a key={partner.name} href={partner.url} target="_blank" rel="noreferrer" initial={reveal(index)} whileInView={{ opacity: 1, y: 0, scale: 1 }} viewport={{ once: true, amount: 0.2 }} whileHover={{ y: -5, borderColor: "rgba(210,180,140,0.7)" }} transition={{ duration: 0.45, ease: "easeOut" }} className="group relative flex min-h-28 flex-col items-center justify-center overflow-hidden border border-cyber-blue/20 bg-white/[0.035] p-5 text-center shadow-[0_8px_25px_rgba(0,0,0,0.18)]">
          ...
          {partner.logo ? (
            ...
            <>
              ...
              <span className="mt-3 font-mono text-[11px] font-bold tracking-wider text-cyber-gray transition-colors group-hover:text-cyber-tan">{"// CONFIRMED SOON"}</span>
            </>
          )}
        </motion.a>)}
      </div>
    ...
    <section id="judges" className="crosshair-corner relative overflow-hidden border border-cyber-blue/10 bg-cyber-dark/20 p-6 md:p-8">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
        {JUDGE_SLOTS.map((slot, index) => <motion.div key={slot} initial={reveal(index)} whileInView={{ opacity: 1, y: 0, scale: 1 }} viewport={{ once: true, amount: 0.15 }} transition={{ duration: 0.45, ease: "easeOut" }} className="overflow-hidden border border-cyber-blue/20 bg-cyber-black/45 p-3">
          <div className="relative flex aspect-[4/3] items-center justify-center overflow-hidden bg-cyber-dark"><span className="font-heading text-3xl text-cyber-tan/40">?</span><span className="absolute left-2 top-2 font-mono text-[10px] tracking-widest text-cyber-blue/60">JUDGE_{String(slot).padStart(2, "0")}</span></div>
          <div className="pt-3"><h3 className="font-heading text-[13px] leading-relaxed text-white uppercase">Judge details coming soon</h3><p className="mt-1 font-mono text-[11px] font-bold tracking-widest text-cyber-tan">TO BE ANNOUNCED</p><p className="mt-2 font-mono text-[12px] leading-relaxed text-cyber-gray">Profile, role and background will be published closer to the event.</p></div>
        </motion.div>)}
      </div>
    </section>
  );
...
export function ContactSection() {
  return (
    <section id="contact_us" className="crosshair-corner relative border border-cyber-blue/10 bg-cyber-dark/20 p-6 md:p-8">
      <div className="mb-6"><span className="font-mono text-[12px] font-bold tracking-widest text-cyber-tan">{"// DIRECT_CONTACTS"}</span><h2 className="mt-2 font-heading text-lg leading-relaxed text-white uppercase md:text-xl">Contact us</h2></div>
      <div className="grid gap-4 lg:grid-cols-3">
        {CONTACTS.map((person, index) => <motion.article key={person.name} initial={reveal(index)} whileInView={{ opacity: 1, y: 0, scale: 1 }} viewport={{ once: true, amount: 0.25 }} transition={{ duration: 0.4 }} className="flex items-center gap-4 border border-cyber-blue/15 bg-cyber-black/40 p-4">
          ...
          <div className="min-w-0"><h3 className="font-heading text-[13px] leading-relaxed text-white uppercase">{person.name}</h3><p className="font-mono text-[11px] font-bold tracking-wide text-cyber-tan">{person.role}</p><div className="mt-2 space-y-1 font-mono text-[12px] text-cyber-gray"><a href={`tel:${person.phone?.replace(/\s/g, "")}`} className="block hover:text-white">{person.phone}</a><a href={`mailto:${person.email}`} className="block truncate hover:text-white">{person.email}</a><a href={person.url} target="_blank" rel="noreferrer" className="block text-cyber-blue hover:text-cyber-tan">LinkedIn ↗</a></div></div>
        </motion.article>)}
      </div>
    </section>
  );

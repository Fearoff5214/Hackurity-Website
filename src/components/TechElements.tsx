...
export function CornerCrosshairs() {
  return (
    <>
      <span className="absolute top-2 left-2 text-[11px] text-cyber-tan/40 select-none pointer-events-none font-mono">+</span>
      <span className="absolute top-2 right-2 text-[11px] text-cyber-tan/40 select-none pointer-events-none font-mono">+</span>
      <span className="absolute bottom-2 left-2 text-[11px] text-cyber-tan/40 select-none pointer-events-none font-mono">+</span>
      <span className="absolute bottom-2 right-2 text-[11px] text-cyber-tan/40 select-none pointer-events-none font-mono">+</span>
    </>
  );
}
...
export function StatusDot({ statusText = "SYSTEM ACTIVE", active = true }: { statusText?: string; active?: boolean }) {
  return (
    <div className="flex items-center gap-2 px-3 py-1 border border-cyber-tan/20 bg-cyber-dark/85 font-mono text-[11px] tracking-widest text-cyber-tan">
...
export function SimulatedLoadingBar({ value = 75, label = "CORE TEMP" }: { value?: number; label?: string }) {
  return (
    <div className="w-full font-mono text-[11px] text-cyber-gray">
...
export function TechTable({ title, headers, rows }: { title: string; headers: string[]; rows: string[][] }) {
  return (
    <div className="w-full border border-cyber-blue/10 bg-cyber-dark/40 overflow-hidden">
      <div className="flex items-center justify-between px-3 py-1.5 border-b border-cyber-tan/25 bg-cyber-tan/5 text-[11px] tracking-wider text-cyber-tan font-bold font-mono">
        <span>// {title}</span>
      ...
      <table className="w-full text-left font-mono text-xs select-none">
        <thead>
          <tr className="border-b border-cyber-blue/10 bg-cyber-black text-cyber-gray/50 text-[11px] uppercase">
            {headers.map((h, i) => (
